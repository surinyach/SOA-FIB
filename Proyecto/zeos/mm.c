...
/* Array with shared memory frames */ 
struct Shared_frame shared_frames[10]; 
...
{
    if ((frame>NUM_PAG_KERNEL)&&(frame<TOTAL_PAGES)&&(phys_mem[frame]==1))
...
/* Fill the shared frames array with frames ID */
void init_shared_frames() {
  int frame = 0; 
  for (int i = 0; i < 10; ++i) {
    frame = alloc_frame(); 
    shared_frames[i].frame = frame; 
    shared_frames[i].marked = 0; 
    shared_frames[i].references = 0; 
  }
}